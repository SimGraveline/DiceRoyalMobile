function scr_pair_draw() {
	...
	// actually gets rendered mid-air) — not on the regular automatic fall pace. Input keeps
	// getting polled while paused (needed for menu nav), so this must explicitly check paused too,
	// otherwise holding S/Down to navigate the pause menu would visually stretch the frozen pair.
	var _falling = !_master_on_ground && !_slave_on_ground && global.input_soft_drop && !global.paused;
